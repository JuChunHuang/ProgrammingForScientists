...
func InitializeGalaxy(num_of_stars int, r, x, y float64) Galaxy {
	g := make(Galaxy, num_of_stars)
	...
	for i := range g {
		...
		speed := 0.5 * math.Sqrt(G*blackhole_mass/dist) // approximation of orbital velocity equation: half of true speed to prevent instability
